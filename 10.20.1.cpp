...
bool cmp(const Point &a, const Point &b)
{
    ...
    if (temp > 0)
        return true;
    if (temp == 0 && dist(a, arr[1]) < dist(b, arr[1]))
        return true;
    return false;
}
...
int main()
{
    ...
    cin >> n;
    int minI = 1;
    ld minY = 2e8, minX = 2e8;
    _rep(i, 1, n)
    {
        cin >> arr[i].x >> arr[i].y;
        if (minY > arr[i].y || (minY == arr[i].y && minX > arr[i].x))
            minI = i, minY = arr[i].y, minX = arr[i].x;
    }
    Point tempzxc;
    tempzxc.x = arr[minI].x, tempzxc.y = arr[minI].y;
    arr[minI].x = arr[1].x, arr[minI].y = arr[1].y;
    arr[1].x = tempzxc.x, arr[1].y = tempzxc.y;
    ...
    _rep(i, 2, n)
    {
        while (posi > 1 && crossMul(st[posi - 1], st[posi], st[posi], arr[i]) <= 0)
            posi--;
        st[++posi] = arr[i];
    }
    st[posi + 1] = arr[1];
