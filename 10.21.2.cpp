...
ld dist(const point &a1, const point &a2)
{
    return sqrt((a1.x - a2.x) * (a1.x - a2.x) + (a2.y - a1.y) * (a2.y - a1.y));
}
...
bool cmp(const point &a, const point &b)
{
    ...
    if (temp > 0)
        return true;
    if (temp == 0 && dist(arr[1], a) < dist(arr[1], b))
        return true;
    return false;
}
...
int main()
{
    ...
    ld duijiaoxian = sqrt((a) * (a) + (b) * (b));
    ld cos_1 = a / duijiaoxian;
    ld cos_2 = -a / duijiaoxian;
    ...
    _rep(i, 1, n)
    {
        ...
        ld temp1 = cos_1 + theta;
        arr[4 * i - 3].x = x + cos(temp1) * duijiaoxian / 2.0;
        arr[4 * i - 3].y = y + sin(temp1) * duijiaoxian / 2.0;

        temp1 = cos_2 + theta;
        arr[4 * i - 2].x = x + cos(temp1) * duijiaoxian / 2.0;
        arr[4 * i - 2].y = y + sin(temp1) * duijiaoxian / 2.0;
