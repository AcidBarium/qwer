...
int main()
{
    ...
    // _rep(i, 1, cnt)
    // {
    //     cout << "st point x y" << st[i].x << ' ' << st[i].y << endl;
    // }

    _rep(i, 2, cnt)
    {
        edge[i - 1].theta = atan2(st[i].y - st[1].y, st[i].x - st[1].x);
        edge[i - 1].id = i;
    }
    edge[cnt].theta = 3.14 / 2.0 * 3.0;
    ...
    {
        cin >> p[i].x >> p[i].y;
        ld th = atan2(p[i].y - st[1].y, p[i].x - st[1].x);
        int posi = lower_bound(edge + 1, edge + cnt + 1, th, cmp3) - edge;
        if (posi > cnt || posi <= 0)
        {
            // cout << "-1" << ednl;
            continue;
        ...
        if (th == edge[i].theta && dis(st[1], p[i]) <= dis(st[1], st[edge[i].id]))
        {
            ans++;
            continue;
        }
        else if (th == edge[i].theta && dis(st[1], p[i]) > dis(st[1], st[edge[i].id]))
        {
            continue;
        }

        // cout << "posi " << posi << " ";
        ld tem = crossMul(st[edge[posi].id - 1], st[edge[posi].id], st[edge[posi].id - 1], p[i]);
        // cout << "temp " << tem << ' ';
        if (tem - 0 >= -1e-4)
        {
            // cout << "1" << endl;
