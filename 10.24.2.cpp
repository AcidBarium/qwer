...
inline void spanlcx()
{
    int temp = rand() % 10000;
    // int temp = 451;
    ld spantemp = 2.0 * 3.1415926535 * temp / 1000.0;
    for (register int i = 1; i <= n; i++)
    {
        ld ju = sqrt(arr[i].x * arr[i].x + arr[i].y * arr[i].y);
        ld theta = acos(arr[i].x / ju);
        if (arr[i].y < 0)
            theta = 2.0 * 3.1415926535 - theta;
        theta += spantemp;
        arr[i].x = ju * cos(theta);
        arr[i].y = ju * sin(theta);
    }
}

inline ll dis(const point2 &a, const point2 &b)
{
...
inline void getAnswer()
{
    for (register int i = 1; i <= n; i++)
    {
        for (register int j = 1; i + j <= n && j <= 260; j++)
        {
            int id1 = arr[i].id, id2 = arr[i + j].id;
            ll trm = dis(brr[id1], brr[id2]);
            ans = ans > trm ? trm : ans;
        }
    }
    // for (register int i = 1; i <= 150; i++)
    // {
    //     ll te = i + 1;
    //     for (register int j = n; j >= max(te, n - 150); j--)
    //     {
    //         int id1 = arr[i].id, id2 = arr[i + j].id;
    //         ll trm = dis(brr[id1], brr[id2]);
    //         ans = ans > trm ? trm : ans;
