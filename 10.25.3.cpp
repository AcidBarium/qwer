...
ld crossMul(const point &a1, const point &a2, const point &b1, const point &b2)
{
...
bool cmp(const point &a, const point &b)
{
    ...
    if (temp > 0)
        return true;
    if (temp == 0 && (dis(a, arr[1]) < dis(b, arr[1])))
        return true;
    return false;
}
...
int main()
{
    ...
    while (asd--)
    {
        ...
        _rep(i, 1, n)
        {
            ld xx, yy, w, h, theta;
            cin >> xx >> yy >> w >> h >> theta;
            theta = theta / 180.0 * 3.14159265358979324;
            sumSqure += w * h;
            ld oriTheta = atan2(h, w);
            ld briLength = sqrt(w * w + h * h) / 2;

            arr[i * 4 - 3].x = xx + briLength * cos(oriTheta - theta);
            arr[i * 4 - 3].y = yy + briLength * sin(oriTheta - theta);

            ld oriTheta2 = 3.14159265358979324 - oriTheta;
            arr[i * 4 - 2].x = xx + briLength * cos(oriTheta2 - theta);
            arr[i * 4 - 2].y = yy + briLength * sin(oriTheta2 - theta);

            ld oriTheta3 = -oriTheta;
            arr[i * 4 - 1].x = xx + briLength * cos(oriTheta3 - theta);
            arr[i * 4 - 1].y = yy + briLength * sin(oriTheta3 - theta);

            ld oriTheta4 = -oriTheta2;
            arr[i * 4].x = xx + briLength * cos(oriTheta4 - theta);
            arr[i * 4].y = yy + briLength * sin(oriTheta4 - theta);
        }

        _rep(i, 1, 4 * n)
        {
            if (ymin > arr[i].y || (ymin == arr[i].y && xmin > arr[i].x))
            {
                imin = i;
                ymin = arr[i].y;
                xmin = arr[i].x;
        ...
        _rep(i, 2, cnt - 1)
        {
            sum += abs(crossMul(st[1], st[i], st[1], st[i + 1]) / 2.0);
        }

        cout << fixed << setprecision(1) << 100.0 * sumSqure / sum << " %" << endl;
    }
