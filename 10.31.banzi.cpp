...
Vector operator-(Vector a, Vector b) { return Vector(a.x - b.x, a.y - b.y); }
Vector operator*(Vector a, ld b) { return Vector(a.x * b, a.y * b); }
Vector operator/(Vector a, ld b) { return Vector(a.x / b, a.y / b); }
Vector operator==(const Vector &a, const Vector &b) { return ldcmp(a.x - b.x) && ldcmp(a.y - b.y); }
...
ld length(const Vector &x)
{
    return sqrt(dot(x, x));
}

ld crossMul(const Vector &a, const Vector &b)
{
...
Point getCP(Point p, Vector v, Point q, Vector w)
{
    Vector u = p - q;
    ld t = crossMul(w, u) / crossMul(v, w);
    return p + v * t;
}
...
int main()
{
    ...
    Point x = getCP(a, v1, c, v2);

    cout << fixed<< setprecision(20) << x.x << ' ' << x.y << endl;
