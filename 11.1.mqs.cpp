...
void BinarySearchTree<T>::deleteByMerging(BinarySearchTreeNode<T> *&node)
{
    ...
    if (node != NULL)
    {
        // 如果被删除节点没有右子树，用其左子树的根节点来代替被删除节点
        ...
        {
            BinarySearchTreeNode<T> *current = getParent(root, tmp);
...
void BinarySearchTree<T>::deleteByCopying(BinarySearchTreeNode<T>*& node)
{
    ...
    if (!node->getRightChild()) {
        ...
        BinarySearchTreeNode<T>* successor = node->getRightChild();
        while (successor->getLeftChild()) {
            successor = successor->getLeftChild();
        }
        node->element = successor->element;
        deleteByMerging(successor);  // Note: Pass by reference here to correctly update the pointer
    }
    if (node == root && !node->getLeftChild() && !node->getRightChild()) {
        root = NULL;
    }
}
