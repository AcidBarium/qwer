...
public:
    ...
    vector<int> isFang;
    vector<int> Fa;
    tu(int lengt) : arr(lengt + 1, vector<int>(lengt + 1)), isFang(lengt + 1), Fa(lengt + 1)
    {
        len = lengt;
        _rep(i, 1, len) Fa[i] = i;
    }
    int len;
    void gouzhao(int n, vector<vector<int>> a)
    {
        len = n;
        _rep(i, 1, n) _rep(j, 1, n) arr[i][j] = a[i][j];
        _rep(i, 1, n) Fa[i] = i, isFang[i] = 0;
    }
    void ze()
    {
        _rep(i, 1, len) isFang[i] = 0;
    }
    ...
    void prim(int p)
    {
        ...
        while (cnt < len)
        {
            _rep(i, 1, len)
            {
                ...
                if (arr[p][i])
                {
                    edge temp(p, i, arr[p][i]);
                    q.push(temp);
                }
            }
            edge dage = q.top();
            while (isFang[dage.to])
            {
                q.pop();
                dage = q.top();
            }

            cout << "from " << dage.from << " to " << dage.to << " value " << dage.value << endl;
            isFang[dage.to] = 1;
            q.pop();
            p = dage.to;
            cnt++;
    ...
    {
        priority_queue<edge> q;
        _rep(i, 1, len) _rep(j, 1, len) // 所有边小根堆
        {
            if (i == j)
                continue;
            if (arr[i][j])
            {
                edge temp(i, j, arr[i][j]);
                q.push(temp);
            }
        }
        while (!q.empty())
        {
            edge temp = q.top();
            q.pop();
            int tofather = find(temp.to);
            int fromfather = find(temp.from);

            if (tofather == fromfather)
                continue;
            Fa[tofather] = fromfather;
            cout << "from " << temp.from << " to " << temp.to << " value " << temp.value << Endl;
        }
    }
};
...
int main()
{
    ...
    _rep(i, 1, n) _rep(j, 1, n) cin >> arr[i][j];
