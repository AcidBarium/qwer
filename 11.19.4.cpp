...
void dij(int p)
{
    ...
    while (!q.empty())
    {
        ...
        {
            if (dis[(*it).to] > dis[now] + (*it).value)
            {
                dis[(*it).to] = dis[now] + (*it).value;
                edge t((*it).to, dis[(*it).to]);
                q.push(t);
            }
        }
    }
...
int main()
{
    ...
    _rep(i, 1, m)
    {
        ...
        cin >> from >> to >> value;
        edge temp(to, value);
        arr[from].vec.push_back(temp);
        edge temp2(from, value);
        arr[to].vec.push_back(temp2);
    }

    dij(s);

    _rep(i, 1, n) cout << dis[i] << ' ';
